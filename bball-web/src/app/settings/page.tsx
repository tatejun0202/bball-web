...
export default function SettingsPage() {
  ...
  async function onDeleteSelected() {
    ...
    const ok = window.confirm(`${selectedIds.length}件の履歴を削除します。よろしいですか？`)
    if (!ok) return
  ...
  async function onDeleteAll() {
    ...
    const ok = window.confirm('すべての履歴を削除します。この操作は取り消せません。')
    if (!ok) return
  ...
  return (
    <main style={{ padding:16 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <Link href="/history" style={{ color:'#9ecbff' }}>＜ Back</Link>
      </div>
      ...
      <section style={{ marginTop:12 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <button
            ...
            style={{ border:'1px solid #666', background:'none', color:'#ddd', borderRadius:8, padding:'6px 10px', cursor:'pointer' }}
          >
            ...
            return (
              <li key={s.id} /* … */>
                ...
                <label htmlFor={cid} style={{ cursor:'pointer' }}>
                  <div style={{ fontWeight:700 }}>{s.note || 'Session'}
                    {!s.endedAt && (
                      <span style={{
                        marginLeft:8, fontSize:11, fontWeight:800,
                        color:'#fff', background:'#d22', padding:'2px 6px', borderRadius:999
                      }}>current</span>
                    )}
                  </div>
                  <div style={{ fontSize:12, color:'#9aa', marginTop:2 }}>{fmt(s.startedAt)}{s.endedAt?` 〜 ${fmt(s.endedAt)}`:''}</div>
                </label>
                <Link href={`/result/${s.id}` as any} style={{ color:'#9ecbff' }}>View ›</Link>
              </li>
            )
          })}
          {sessions.length === 0 && (
